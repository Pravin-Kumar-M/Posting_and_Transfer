...
<body>
    ...
    <div class="container mt-5">
        ...
        <div class="shadow-lg p-5 mb-4 rounded-5 mx-auto" style="max-width: 600px;">
            <h1 class="text-center mb-4">Office</h1>

            <form action="{{ route('admin.office.store') }}" method="post" autocomplete="off">
                @csrf
                <div id="dynamicRows">
                    <div class="row">
                        <div class="col-12  col-sm-9">
                            <div class="form-floating mt-3 mb-3">
                                <input type="text" class="form-control" placeholder="Enter office" name="office_name[]" required oninput="capitalizeInput(event)">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <!-- Form to delete office -->
                                        <form id="delete-form-{{ $office->id }}" action="{{ route('admin.office.destroy', $office->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-danger" onclick="confirmDelete({{ $office->id }})">Remove</button>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
    ...
    <script>
        function checkAndAddRow() {
            const inputFields = document.querySelectorAll('input[name="office_name[]"]');
            let allFilled = true;
        ...
        function confirmDelete(officeId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Submit the form for the specific office ID
                    document.getElementById(`delete-form-${officeId}`).submit();
                }
            });
        }
    </script>
