...
<body>
    ...
    <div class="container mt-5">
        ...
        <div class="shadow-lg p-5 mb-4 rounded-5 mx-auto" style="max-width: 600px;">
            <h1 class="text-center mb-4">Designation</h1>

            <form action="{{ route('admin.cadre.store') }}" method="post" autocomplete="off">
                @csrf
                <div id="dynamicRows">
                    <div class="row">
                        <div class="col-12  col-sm-9">
                            <div class="form-floating mt-3 mb-3">
                                <input type="text" class="form-control" placeholder="Enter Designation" name="cadre_name[]" required oninput="capitalizeInput(event)">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <!-- Form to delete cadre -->
                                        <form id="delete-form-{{ $cadre->id }}" action="{{ route('admin.cadre.destroy', $cadre->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-danger" onclick="confirmDelete({{ $cadre->id }})">Remove</button>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
    ...
    <script>
        function checkAndAddRow() {
            const inputFields = document.querySelectorAll('input[name="cadre_name[]"]');
            let allFilled = true;
        ...
        <div class="row align-items-center">
            <div class="col">
                <div class="form-floating mt-3 mb-3">
                    <input type="text" class="form-control" placeholder="Enter Cadre" name="cadre_name[]" required oninput="capitalizeInput(event)">
                    <label>Enter Designation</label>
                </div>
            </div>
            <div class="col-auto">
        ...
        function confirmDelete(cadreId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Submit the form for the specific cadre ID
                    document.getElementById(`delete-form-${cadreId}`).submit();
                }
            });
        }
    </script>
